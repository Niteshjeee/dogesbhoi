...
const CONDITIONS = new Set([
  'seen','safe','injured','hungry',
  'with_puppies','afraid','needs_help'
]);
...
export async function onRequestPost({ request, env }) {
  ...
  const dog = await env.DB.prepare(
    'SELECT id FROM dogs WHERE id=? AND active=1'
  ).bind(dogId).first();
  ...
  const perDog = await env.DB.prepare(`
    SELECT COUNT(*) c
    FROM sightings
    WHERE reporter_hash=? AND dog_id=? AND created_at>?
  `).bind(reporterHash, dogId, tenMinAgo).first();
  ...
  if (Number(perDog?.c || 0) >= 3 || Number(global?.c || 0) >= 20) {
    return json({ error: 'Too many reports. Please try later.' }, 429);
  }

  const previous = await env.DB.prepare(`
    SELECT latitude,longitude,created_at,reporter_hash
    FROM sightings
    WHERE dog_id=? AND moderation_status='accepted'
  ...
  if (
    previous &&
    previous.reporter_hash !== reporterHash &&
    Date.now() - Number(previous.created_at) < 6 * 60 * 60_000
  ) {
    corroborator =
      distanceKm(
  ...
  await env.DB.prepare(`
    INSERT INTO sightings(
      id,dog_id,latitude,longitude,accuracy_m,
      condition,note,confidence,moderation_status,
      risk_reason,reporter_hash,created_at
    )
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
  `).bind(
    id,
